...
//和页面异步开始的
    asyncData({ params, error }) {
        return course.getCourseDetilsInfo(params.id).then(response => {
    ...
    data() {
        ...
        methods:{
            //获取课程详情
            initCourseInfo() {
            course.getCourseDetilsInfo(this.courseId)
            .then(response => {
            ...
            isBuyCheck(){
                ...
                this.Ids.mid = this.loginInfo.id
                order.isBuyCheck(this.Ids).then(res =>{
                    console.log(this.Ids)
                    this.isbuyCourse = res.data.data.flag
                })
    }
    }
};
